...
> **Mask-source revision (2026-07-18, evidence `docs/real-plan-eval-01.md` §5–§6):** step 1's luminance threshold cannot separate walls from annotation on real CAD-plotted sheets. On the first real fixture, dimension lines, structural grid lines, door swings, furniture, and stroked SHX text share ink with walls at every threshold and every resolution (ppf 10 and ppf 20 produced identical failure signatures); detected rooms fragment on interior annotation or leak when thin wall types drop below threshold, and no single `(wall_threshold, door_gap_ft)` pair works across rooms of one sheet. **The primary mask source for CAD PDFs is therefore vector-filtered**: A3.3's extracted segments, filtered by stroke width (data-driven threshold from the page's stroke-width histogram), rasterized onto the same `px_per_foot` grid — then steps 2–5 of this pipeline run unchanged on that mask. The luminance threshold of step 1 remains as the **fallback for scanned PDFs** (no vectors → empty segment set → raster mask). See the final spec §9 decision log ("Level-1 wall mask source").
...
> **Status: REQUIRED (promoted 2026-07-18, evidence `docs/real-plan-eval-01.md` §2).** The very first real fixture sheet was a 36"×24" drawing plotted fit-to-page onto letter paper: its printed scale text ("3/16\" = 1'-0\"", fpi 5.333) was wrong for the PDF by **3.27×**; two-point calibration against a printed overall dimension recovered fpi 17.4536, matching the 36×24→letter plot theory to 0.01%. Trusting the title-block text would have made every quantity on the sheet ~3.27× undersize. Verification is therefore **not a differentiator to sequence later — it is a gate**: any scale set from printed scale text (AI-read or human-read) or from a named preset MUST pass verification against printed dimensions (A5.1/A5.2 machinery) before quantities from that page are trusted or exportable. A calibrated scale (`source:'calibrated'`) is self-verifying by construction — it *is* a measurement of a printed dimension — but still receives the A5.2 pass when available, and its `ScaleState.verification` record is populated either way.
