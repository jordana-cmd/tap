...
**Update (2026-07-19): the full-size fixture now exists.** Fixture 002 — a full-size 22"×34" set, 74 pages (client name withheld from committed docs per the confidentiality rule) — is loaded locally. Its stroke-width histogram is **0 / 0.36 / 0.72 / 0.84 / 1.74 pt**: real pen diversity, versus this sheet's collapsed 0.12/0.24, confirming the plot-artifact hypothesis about width collapse (note also the 0-pt bucket — PDF zero-width "thinnest device line" strokes — which the current filter treats as thinnest-and-droppable). The §4 width-filter verdict is therefore **ready to be re-tested** against fixture 002. ~~That re-test has NOT yet been run; until it is, the verdict remains unsettled in both directions.~~ **Re-test complete: see `docs/real-plan-eval-03.md` — the verdict is upheld with a corrected cause (pen tables encode drafting convention, not semantics; hatch-structure filtering promoted to next build item).**
...
1. **Seed-on-dilated-mask defect now blocks a whole room** (Multi Purpose 100 fails at every vector setting; 5 of 7 rooms fail at dg 3.5). Carried from eval-01 A1, still the single highest-leverage fix: validate the seed against the raw mask and nudge to the nearest free dilated-mask pixel.
   **FIXED** in commit `8158667` (raw-mask validation + bounded BFS nudge that never crosses visible ink; fully-closed regions report the new `SEED_TRAPPED` with lower-door-gap guidance). Re-measured on this sheet at min_width 0.18: **Multi Purpose 100 unblocked** — 97.3 SF at dg 3.5, 110.7 SF at dg 2.0/1.0 (a fragment, per §4's same-width-ink verdict, but a result instead of an error); the dg 3.5 row went from six seed failures to three results (concession 50.0, multi 97.3, circulation 25.9) plus three honest `SEED_TRAPPED`s (office, life guards, mechanical — rooms whose open floor is fully closed at r = 18 px; lowering door_gap resolves them, matching §3). Every previously-succeeding cell reproduced byte-identically, confirming the fast-path guarantee on real data.
