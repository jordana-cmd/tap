...
| CRDT library | `yrs` (Yjs Rust port) — **LOCKED** by the Engine Foundation step-5 spike (see `docs/crdt-spike-report.md`, 2026-07-18, yrs 0.27.3 vs automerge 0.10.0) | Data: 15 B vs 119 B per-vertex trace update (7.9×, the highest-volume message we emit), native UndoManager vs none, 127 KB vs 373 KB gzipped wasm, y-websocket-compatible relay path vs build-our-own sync; weighted score 8.42 vs 5.11. Concurrent delete-while-editing silently discards in-progress work in BOTH libraries → the document model MUST soft-delete measurements (`deleted:{by,at}` field, UI filter + restore, retention-window hard-delete at compaction), never remove map entries. Revisit automerge only if register-conflict recovery becomes a product feature or automerge gains native undo AND closes the wire-overhead gap to ≤2× — re-run the committed spike to check. |
...
| Level-1 wall mask source | Vector-filtered segments (stroke-width filter, data-driven threshold) primary for CAD PDFs; raster luminance threshold fallback for scans | Raster-only rejected — eval-01 (`docs/real-plan-eval-01.md`): annotation (dims, grids, swings, SHX text, furniture) is indistinguishable from walls by luminance at any threshold or resolution; rooms fragment or leak and no single parameter pair works across one sheet. See addendum §A3.1 mask-source revision (2026-07-18). |
